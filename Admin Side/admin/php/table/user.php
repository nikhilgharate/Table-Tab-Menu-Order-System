...
<?php
  ...
  //echo "emtpy";
  $sql="INSERT INTO `usermast` (`tid`, `tno`, `tpass`) VALUES (NULL, '$tname', '$tpass');";
	...
	function getRecords($params) {
       $qtot = mysqli_query($this->conn,"SET NAMES utf8"); 
		...
		if( !empty($params['searchPhrase']) ) {   
			$where .=" Where ";
			$where .=" ( tid LIKE '".$params['searchPhrase']."%' "; 
			$where .=" OR tno LIKE '".$params['searchPhrase']."%'";   
			 
			
			$where .=" OR tpass LIKE '".$params['searchPhrase']."%' )";
	   }
	   ...
	   // getting total number records without any search
		$sql = "SELECT *,tid as ID FROM `usermast` ";
		$sqlTot .= $sql;
		$sqlRec .= $sql;
...
function updateEmployee($params) {
   ...
   $sql = "UPDATE `usermast` SET `tno` = '".$params["tname"]."',
		`tpass` = '" . $params["tpass"]."'
		WHERE tid='".$params["edit_ID"]."'";
...
//echo $_FILES['photo']['name'];
 ...
 die('{"msg":"Updated successfully","type":"success","error":"1"}');
	...
	function deleteEmployee($params) {
		...
		$data = array();
		//print_R($_POST);die;
		$sql = "delete from `usermast` WHERE tid='".$params["id"]."'";
