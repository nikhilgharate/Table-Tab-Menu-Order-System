   ...
   <div class="content-page">
	...
	<div class="table-responsive">
		...
		<table id="employee_grid" class="table table-bordered table-hover" width="60%" cellspacing="0" data-toggle="bootgrid">
			...
			<thead>
				<tr>
					<th data-column-id="ID" data-type="numeric" data-identifier="true">Order No</th>
         
          <th data-column-id="odate">Date</th>
          <th data-column-id="otime">Time</th>
          <th data-column-id="tabno">Table No.</th>
          <th data-column-id="amount">Amount</th>
          <th data-column-id="ccno">Card No</th>
          <th data-column-id="mobno">Mobile</th>
          <th data-column-id="otp">OTP</th>
		...
		formatters: {
		        "commands": function(column, row)
		        {
		            return "<button type=\"button\" class=\"btn btn-xs btn-info command-edit\" data-row-id=\"" + row.ID + "\"><span class=\"glyphicon glyphicon-edit\"></span> Edit</button> " + 
		                "<button type=\"button\" class=\"btn btn-xs btn-danger command-delete\" data-row-id=\"" + row.order_no + "\"><span class=\"glyphicon glyphicon-trash\"></span> Delete</button>";
		        }
		    }
...
console.log(g_id);
  ...
  $("#user_edit").load("php/order/order_edit.php?id="+idd);       
    ...
    }).end().find(".command-delete").on("click", function(e)
    {
		...
		var conf = confirm('Delete ' + $(this).data("row-id") + ' items?');
					//alert(conf);
                    if(conf){
                                $.post('php/order/order.php', { id: $(this).data("row-id"), action:'delete'}
                                    , function(){
